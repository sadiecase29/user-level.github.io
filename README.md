# user-level.github.io
# CIT Minor Portfolio

Welcome to my CIT Minor portfolio. Here, you'll find links to my labs and projects, each with a brief description of their purpose and content.

## Labs and Projects

- [Lab 1: Install Required Software and Practice Using It](https://github.com/sadiecase29/CIT281-lab1)
  - Purpose: Get to know the software and tools that we will be using for the course.
# CIT 281 Lab 1

## Purpose

The purpose of this lab was to: 
- Install and setup your personal computer with the software tools required for this course
- Practice using software tools
- Practice terminal or shell commands
- Practice keyboard shortcuts
- Create your first Node.js server-side JavaScript file

## Technologies I Used

- Chrome
- Visual Studio Code (VSCode)
- VSCode Extensions:
- Prettier - Code formatter
- ESLint
- GitHub Pull Requests and Issues
- Node.js (including npm)
- git
- Postman

## What I Learned

- How to create files in the terminal
- Different shortcuts
- How to run Node.js files
- Software Environment Setup
- Code Editor Proficiency
- Command Line Proficiency
- Keyboard Efficiency

- [Lab 2: Basic CLI and Git](https://github.com/sadiecase29/CIT281-lab-02)
  - Purpose: Get an introduction to git and repositories.
   # CIT 281 Lab 2

## Purpose

The purpose of this lab was to: 
- Use CLI commands for directory and file manipulation
- Use CLI commands for git commands
- Setup git user credentials
- New git repository
- Initial JavaScript file
- Add file to git
- Ignoring files
- Using diff to see changes
- Using wildcards
- Cloning cloud repos locally
- Branch
- Repo files

## Technologies I Used

- Command Line Interface (CLI)
- git
- JavaScript and Node.js
- .gitignore

## What I Learned

- Setting Up Git User Credentials
- Creating and Initializing a Git Repository
- Managing Files with Git
- How to use .gitignore
- Cloning Remote Repositories
- Working with Branches
- Listing Tracked Files

- [Project 1: Gain experience with CLI commands and VScode](https://github.com/sadiecase29/CIT281-p1)
  - Purpose: Use CLI commands in the terminal to create and execute javascript files.
# CIT 281 Project 1

## Purpose

The purpose of this lab was to: 
- Gain experience accessing your operating system's command line interface (CLI)
- Gain experience working with CLI commands
- Gain experience working with Visual Studio Code (VSCode)
- Gain experience writing and executing non-web server Node.js JavaScript code

## Technologies I Used

- Command Line Interface (CLI)
- Git
- JavaScript and Node.js

## What I Learned

- How to work with CLI Commands 
- How to use Git and Version Control
- How to manipulate JavaScript Programming

- [Lab 3: Practice with syntax with objests and arrays](https://github.com/sadiecase29/CIT281-lab-03)
  - Purpose: Focus on destructuring objects and arrow function expressions.
# CIT 281 Lab 3

## Purpose

The purpose of this lab was to: 
- Practice refactoring JavaScript code to use more modern syntax
- Practice destructuring an object
- Practice converting normal functions into arrow function expressions
- Examine using for..in  syntax with objects
- Practice using for..of syntax with arrays
- Practice creating a Node.js compatible code module using module.exports
- Practice importing a Node.js compatible code module using require
- Examine code using the spread operator

## Technologies I Used 

- JavaScript
- Node.js

## What I Learned

- Object Destructuring
- Arrow Functions
- for..in and for..of Loops
- Spread Operator
- Module Exports and Imports

- [Project 2: Non-web server Node.js](https://github.com/sadiecase29/CIT281-p2)
  - Purpose: Gain experience writing and executing non-web server Node.js JavaScript code.
# CIT 281 Project 2

## Purpose

The purpose of this lab was to: 
- Gain experience using git via your CLI and Visual Studio Code (VSCode) Source Control
- Gain experience writing and executing non-web server Node.js JavaScript code
- Practice refactoring JavaScript code

## Technologies I Used

- Command Line Interface (CLI)
- Git
- JavaScript and Node.js
- .gitignore

## What I Learned

- I got more experience with the non-web server and was able to see how it works
- Learned how to refactor code more efficiently
- How to use Git for Version Control
- How to use VSCode for Development and Git Operations
- How to create and use .gitignore

- [Lab 4: Introduction to Fastify](https://github.com/sadiecase29/CIT281-lab-04)
  - Purpose: Learn to Create initial Fastify Node.js web server.
# CIT 281 Lab 4

## Purpose

The purpose of this lab was to: 
- Create initial Fastify Node.js web server
- Initialize as a Node.js project folder using Node Package Manager (npm)
- Add Fastify to project using npm, and test using Visual Studio Code (VSCode)
- Add git repo, exclude node_modules folder from git, make commits
- Fix MIME error, test, and commit
- Add a second route with query parameters, test, and commit

## Technologies I used 

- Node.js
- Fastify
- npm (Node Package Manager)
- Git
- JavaScript

## What I Learned

- Learned how to use node modules
- Learn what fastify does and different ways to use it
- Found more efficient ways to fix errors
- How to manage Node.js Project with npm
- How to Fix MIME Type Errors
- How to handle Query Parameters in Fastify

- [Lab 5: Introduction to Postman](https://github.com/sadiecase29/CIT281-lab-05)
  - Purpose: Use Postman to test server GET routes, Add POST handling to server application and respond with JSON, Use Postman and POST request.
# CIT 281 Lab 5

## Purpose

The purpose of this lab was to: 
- Download and install Postman
- Create a CIT 281 collection and folders
- Create a Node.js and fastify server application with GET and respond with JSON
- Add array of students object
- Use Postman to test server GET routes
- Add POST handling to server application and respond with JSON
- Use Postman and POST request

## Technologies I Used

- Node.js
- Fastify
- Postman
- npm (Node Package Manager)
- JavaScript

## What I Learned
- Learned how to navigate the Postman app
- Got introduced to GET routes
- Got introduced to the POST request
- Setting Up and Testing the Fastify Web Server
- How to Use Postman for API Testing
- How to Manage Project Dependencies with npm
- How to Handle JSON Data

- [Project 3: Practice creating and using Code Modules](https://github.com/sadiecase29/CIT281-p3)
  - Purpose: Gain experience writing and executing web server Node.js JavaScript code using VSCode.
# CIT 281 Project 3

## Purpose

The purpose of this lab was to: 
- Gain experience interpreting functional descriptions and specifications to complete an assignment
- Gain experience breaking a project into manageable components
- Gain experience writing and executing non-web server Node.js JavaScript code using VSCode
- Practice creating and using code modules
- Practice refactoring using modern JavaScript syntax
- Gain experience writing and executing web server Node.js JavaScript code using VSCode
- Gain experience using Fastify with the GET verb, routes, and query parameters
- Gain experience loading a file and using as a web page

## Technologies I Used

- Node.js
- Fastify
- JavaScript
- npm (Node Package Manager)
- Git
- HTML
- File System (fs) Module

## What I Learned
- How to Create and Use Code Modules
- How to Set Up and Test Fastify Web Server
- How to Use npm for Dependency Management
- Gained experience with HTML and Web Page Serving

- [Lab 6: Introduction to Creating Classes](https://github.com/sadiecase29/CIT281-lab-06)
  - Purpose: Learn how to create book and library classes.
# CIT 281 Lab 6

## Purpose

The purpose of this lab was to: 
- Join GitHub and work through Introduction to GitHub course
- Create lab file
- Classes overview
- Create and test Book class
- Create and test Library class
- Add at least two more books to the library
- Add ISBN and a delete book method

## Technologies I Used

- Node.js
- Fastify
- JavaScript
- npm (Node Package Manager)
- Git
- HTML
- File System (fs) Module

## What I Learned
- How to Create and Use Code Modules
- How to Set Up and Test Fastify Web Server
- How to Use npm for Dependency Management
- Gained experience with HTML and Web Page Serving
- [Project 4: Gain experience with REST Server and GET Verb](https://github.com/sadiecase29/CIT281-p4)
  - Purpose: Gain experience using Fastify with the GET verb, routes, and route parameters.
# CIT 281 Project 4
## Purpose

The purpose of this lab was to: 
- Gain experience interpreting functional descriptions and specifications to complete an assignment
- Gain experience breaking a project into manageable components
- Gain experience writing and executing non-web server Node.js JavaScript code using VSCode
- Practice creating and using code modules
- Practice using modern JavaScript syntax
- Gain experience writing and executing Node.js REST API server using VSCode
- Gain experience using Fastify with the GET verb, routes, and route parameters
- Gain experience working with static data
- Gain experience testing code module without using a web server
- Gain experience using Postman to test web server routes
- Gain experience working with JSON

## Technologies I Used

- Node.js
- Fastify
- JavaScript
- npm (Node Package Manager)
- Git

## What I Learned
- Gained more experience with using npm for Dependency Management
- Gained more experience with JSON
- Gained more experience with Postman
- How to better manipulate HTML and Web Page Serving

- [Lab 7: Gain experience with GitHub](https://github.com/sadiecase29/CIT281-lab-07)
  - Purpose: Create your own GitHub Personal Website Using GitHub Pages and Create lab GitHub code repository.
# CIT 281 Lab 7
## Purpose

The purpose of this lab was to: 
- Create your own GitHub Personal Website Using GitHub Pages
- Create lab GitHub code repository
- Clone your GitHub repository to your local system
- Create lab JavaScript file, and initialize folder as a Node.js folder
- Update and commit lab JavaScript file to practice with error handling
- Push your changes to GitHub

## Technologies I Used

- GitHub
- Node.js
- HTML

## What I Learned
- Working with Github Repositories 
- More work with Javascriot errors 
- More work with erroes in the VScode terminal 

- [Project 5: Gain more experience working with code modules](https://github.com/sadiecase29/CIT281-p5)
  - Purpose: working with Array map() and forEach() methods, the concept of a gaming loop, and objects and arrays.
# CIT 281 Project 5 
## Purpose

The purpose of this lab was to: 
- Gain experience working with classes, including class constructors and methods
- Gain experience interpreting existing code to design a new class, including constructor and methods
- Gain more experience working with code modules
- Gain experience executing and debugging Node.js code using VSCode, including module breakpoints
- Gain more experience working with Array map() and forEach() methods
- Gain experience working with the concept of a gaming loop
- Gain more experience working with objects and arrays

## Technologies I Used

- JavaScript
- Modern JavaScript Syntax
- Array Methods
- Node.js
- Gaming Loop Concept
- Random Number Generator


## What I Learned
- How to manipulate classes 
- How to create and manipulate Arrays
- How to use the gaming loop
- More experience with debugging

- [Lab 8: Gain more experience with node-fetch and Fastify](https://github.com/sadiecase29/CIT281-lab-08)
  - Purpose: Install node-fetch package and add fastify starter code.
# CIT 281 Lab 8
## Purpose

The purpose of this lab was to: 
- Create lab file and initialize Node.js folder
- Install node-fetch package
- Add fastify starter code
- Add fastify and node-fetch require statements
- Fetch JSONPlaceholder data

## Technologies I Used

- Node.js
- Fastify
- Node-fetch
- JSONPlaceholder
- JavaScript

## What I Learned
- How to Integrate External APIs
- How to Handle Asynchronous Operations
- How to use Node-fetch

- [Project 6: Interpreting functional descriptions and specifications to complete an assignment](https://github.com/sadiecase29/CIT281-p6)
  - Purpose: Gain experience creating and working classes with inheritance, creating and working with classes, and debugging code.
# CIT 281 Project 6
## Purpose

The purpose of this lab was to: 
- Gain experience creating and working classes with inheritance
- Gain more experience creating and working with classes
- Gain more experience debugging code
- Gain more experience using a generic block of code to process data
- Gain more experience interpreting functional descriptions and specifications to complete an assignment
- Gain more experience writing and executing non-web server Node.js JavaScript code using VSCode
- Practice using modern JavaScript syntax
- Gain more experience working with static data

## Technologies I Used

- Node.js
- Math Functions

## What I Learned
- How to Debug and Test Javascript code 
- Got more practice processing Data with Classes

- [Lab 9: Create a Release Based Workflow in GitHub](https://github.com/sadiecase29/CIT281-lab-09)
  - Purpose: Create Learning Lab course/repo within your GitHub account.
# CIT 281 Lab 9
## Purpose

The purpose of this lab was to: 
- Create Learning Lab course/repo within your GitHub account
- Practice exiting and returning to the course from course repo
- Work through course
- Download completed course repo as deliverable

## Technologies I Used

- GitHub
- Git Version Control
- GitHub Pages
- Lab Workflow

## What I Learned

- Gained more practice with Github repositories 



 



